...
""" IP subnet calculator
    ...
    TODO: install in /ms/dist/python/PROJ/ipcalc/0.1
"""
...
class IP(object):
    ...
    def _itodq(self, n):
        ...
        """
        if self.v == 4:
            return '.'.join(map(str, [(n>>24) & 0xff, (n>>16) & 0xff,
(n>>8) & 0xff, n & 0xff]))
        else:
            n = '%032x' % n
    ...
    def to_ipv6(self, type='6-to-4'):
        ...
        if self.v == 4:
            if type == '6-to-4':
                return IP(0x20020000000000000000000000000000L | long(self)<<80,
                          version=6)
            elif type == 'compat':
...
class Network(IP):
    ...
    def netmask(self):
        ...
        """
        if self.version() == 4:
            return IP((0xffffffffL >> (32-self.mask)) << (32-self.mask),
                version=self.version())
        else:
            return IP((0xffffffffffffffffffffffffffffffffL >>
                       (128-self.mask)) << (128-self.mask),
                version=self.version())
    ...
    def broadcast(self):
        ...
        if self.version() == 4:
            return IP(long(self.network()) | (0xffffffff - long(self.netmask())),
                      version=self.version())
        else:
            return IP(long(self.network()) | (
                0xffffffffffffffffffffffffffffffffL - long(self.netmask())),
                      version=self.version())
