...
insert into hostlifecycle (id, name, creation_date, comments) values(
  select id, name, creation_date, comments from status
);
...
insert into clusterlifecycle (id, name, creation_date, comments) values(
 select id, name, creation_date, comments from hostlifecycle where
  name in ('ready', 'build', 'decommissioned', 'rebuild')
);
create sequence clusterlifecycle_id_seq start with 20 increment by 1 nocache nocycle;
