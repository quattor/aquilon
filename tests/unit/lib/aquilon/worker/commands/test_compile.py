...
with mock.patch('aquilon.aqdb.db_factory.DbFactory', autospec=True):
    from aquilon.worker.commands import compile


class TestCommandCompile(unittest.TestCase):
    @staticmethod
    @mock.patch.object(compile.CommandCompile, '__init__')
    def get_command_instance(mock_init):
        mock_init.return_value = None
        command = compile.CommandCompile()
        return command

    @mock.patch.object(compile, 'CompileKey')
    @mock.patch.object(compile.CommandCompile, '_get_template_domain')
