...
class CommandAddChassis(CommandAddLocation):

    required_parameters = ["name", "rack"]

    @add_transaction
    @az_check
    def render(self, session, name, rack, fullname, comments, **arguments):
        return CommandAddLocation.render(self, session=session, name=name,
                type='chassis', fullname=fullname,
                parentname=rack, parenttype='rack',
